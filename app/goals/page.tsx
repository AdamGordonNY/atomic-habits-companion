...
import { fetchGoalsData, fetchHabitsData } from "@/lib/profile-data";
...
export default async function GoalsPage() {
  const [{ isAuthed, data }, habitsResult] = await Promise.all([
    fetchGoalsData(),
    fetchHabitsData(),
  ]);
  const notes = isAuthed ? await actionGetNotesForProfileEntity("goals", data?.id ?? null) : [];
  return <div className="mx-auto max-w-3xl space-y-6 px-5 py-8"><GoalsSection data={data} habits={habitsResult.data} showRouteLink={false} /><ProfileNotesPanel entityType="goals" entityId={data?.id ?? null} initialNotes={notes} /></div>;
}
